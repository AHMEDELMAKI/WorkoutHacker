...
export function buildSystemPrompt(request: NormalizedWorkoutRequest): string {
  const rules = `You are a strength and conditioning planner that outputs only valid JSON.
...
18) Return only JSON matching the provided schema. No markdown.`;
  ...
  const userContext = JSON.stringify(request, (key, value) => (key === 'naturalLanguageRequest' ? undefined : value));
