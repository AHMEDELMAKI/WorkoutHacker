...
export interface SignalCollectionStatus {
  ready: boolean;
...
export class DataAggregator {
  ...
  private firstHRAt: number | null = null;
  private firstEMGAt: number | null = null;
  private firstVelocityAt: number | null = null;
  ...
  ingestHeartRate(reading: HeartRateMeasurement): void {
    ...
    this.firstHRAt ??= reading.timestamp;
  }

  ingestEMG(sample: EMGSample): void {
    this.latestEMG = sample;
    this.firstEMGAt ??= sample.timestamp;
  }

  ingestVelocity(reading: VelocityReading): void {
    this.latestVelocity = reading;
    this.firstVelocityAt ??= reading.timestamp;
  }

  noteVelocitySignal(timestamp: number): void {
    this.firstVelocityAt ??= timestamp;
  }
  ...
  getCollectionStatus(minDurationMs: number): SignalCollectionStatus {
    const now = Date.now();
    const hrDuration = this.firstHRAt !== null ? Math.max(0, now - this.firstHRAt) : 0;
    const emgDuration = this.firstEMGAt !== null ? Math.max(0, now - this.firstEMGAt) : 0;
    const velocityDuration = this.firstVelocityAt !== null ? Math.max(0, now - this.firstVelocityAt) : 0;

    const hasSamples = {
      hr: this.latestHR !== null,
      emg: this.latestEMG !== null,
      velocity: this.firstVelocityAt !== null,
    };
    ...
    const ready =
      ...
      hasSamples.emg &&
      hasSamples.velocity &&
      hrDuration >= minDurationMs &&
      emgDuration >= minDurationMs &&
      velocityDuration >= minDurationMs;
