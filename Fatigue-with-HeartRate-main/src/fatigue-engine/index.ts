...
import { DataAggregator, SignalCollectionStatus } from '../aggregator';
...
export class FatigueEngine {
  ...
  private assessmentListeners: Array<(status: SignalCollectionStatus) => void> = [];
  ...
  on(event: 'assessment-data', listener: (status: SignalCollectionStatus) => void): this;
  on(
    event: 'state' | 'progress' | 'assessment-data',
    listener: ((e: StateEvent) => void) | ((e: RestProgressEvent) => void) | ((status: SignalCollectionStatus) => void)
  ): this {
    ...
    if (event === 'assessment-data') this.assessmentListeners.push(listener as (status: SignalCollectionStatus) => void);
    return this;
  }
  ...
  private async assessPhase(depth = 0): Promise<void> {
    ...
    await this.waitForAssessmentData();
    if (!this.workoutActive) return;

    this.transition('assessing', 'Evaluating fatigue signals from HR, EMG, and IMU…');
  ...
  private async waitForAssessmentData(): Promise<void> {
    const minDurationMs = this.config.minAssessmentDataMs ?? 30_000;
    const pollIntervalMs = this.config.assessmentPollIntervalMs ?? 1000;
    const timeoutMs = this.config.assessmentTimeoutMs ?? 90_000;
    ...
    return new Promise(resolve => {
      const tick = () => {
        ...
        const status = this.aggregator.getCollectionStatus(minDurationMs);
        this.assessmentListeners.forEach(fn => fn(status));

        if (status.ready || (Date.now() - startedAt) >= timeoutMs) {
          resolve();
          return;
        }
        ...
        const remainingSeconds = Math.max(
          ...
          Math.ceil(
            Math.max(
              minDurationMs - status.durationsMs.hr,
              minDurationMs - status.durationsMs.emg,
              minDurationMs - status.durationsMs.velocity,
            ) / 1000,
          ),
        );
        ...
        const message = missingSignals.length > 0
          ? `Collecting ${minSeconds}s of sensor data before fatigue assessment. Waiting for ${missingSignals.join(', ')}.`
          : `Collecting ${minSeconds}s of HR, EMG, and IMU data before fatigue assessment. ${remainingSeconds}s remaining.`;

        this.transition('assessing', message);
