...
export const CameraHeartRateComponent: React.FC<CameraHeartRateProps> = ({
  ...
  const [isTorchOn, setIsTorchOn] = useState(useTorch);
  ...
  const startBridge = useCallback(async () => {
    ...
    if (useTorch) {
      try {
        await nativeModule.setTorchEnabled(true);
        setIsTorchOn(true);
      } catch (err) {
        // Torch may not be supported or permission denied; notify but continue
        const msg = err instanceof Error ? err.message : 'Unable to enable torch';
        onError?.(msg);
        setIsTorchOn(false);
      }
    }
  ...
  // Keep the native torch aligned with the requested prop after initialization.
  useEffect(() => {
    ...
    (async () => {
      try {
        await nativeModule.setTorchEnabled(useTorch);
        if (mounted) setIsTorchOn(useTorch);
      } catch (err) {
        if (mounted) setIsTorchOn(false);
      }
    })();
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Centered instruction box for PPG capture */}
      {bridgeStatus === 'running' && (
        <View style={styles.captureOverlay} pointerEvents="none">
          <View style={styles.captureBox} />
          <Text style={styles.captureText}>Place your finger over the camera and flash</Text>
          <Text style={styles.captureSubText}>{isTorchOn ? 'Flash enabled' : 'Flash unavailable'}</Text>
        </View>
      )}
