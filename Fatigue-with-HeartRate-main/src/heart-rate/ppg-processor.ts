...
import { clamp, findPeaks, mean } from '../utils/math';
...
export class PPGProcessor {
  ...
  private estimate(): HeartRateMeasurement | null {
    ...
    // Confidence: prefer consistent BPM spacing, but do not over-penalize short sample windows.
    const variance = validBPMs.reduce((acc, b) => acc + (b - rawBPM) ** 2, 0) / validBPMs.length;
    const varianceScore = clamp(1 - variance / 300, 0, 1);
    const sampleCountScore = clamp(validBPMs.length / 4, 0, 1);
    const confidence = (0.55 * varianceScore) + (0.45 * sampleCountScore);
