...
class CameraHeartRateModule(private val context: ReactApplicationContext) : ReactContextBaseJavaModule(context) {
  ...
  private var fallbackTorchCameraId: String? = null
  ...
  fun start(options: ReadableMap?, promise: Promise) {
    ...
    val targetFps = if (options?.hasKey("targetFps") == true) options.getInt("targetFps") else 30
    val sampleRate = if (options?.hasKey("sampleRate") == true) options.getInt("sampleRate") else targetFps
    val analysisWindowSec = if (options?.hasKey("analysisWindowSec") == true) options.getInt("analysisWindowSec") else 5
  ...
  fun setTorchEnabled(enabled: Boolean, promise: Promise) {
    ...
    if (activeCamera == null) {
      // Camera is transitioning (bind/unbind). Treat as non-fatal; caller may retry.
      promise.resolve(null)
      return
    }

    if (!activeCamera.cameraInfo.hasFlashUnit()) {
      val fallbackOk = setTorchWithCameraManagerFallback(enabled)
      if (fallbackOk) {
        emitState(if (enabled) "torch-on" else "torch-off")
        promise.resolve(null)
        return
      }
      emitError("Torch is not available on this camera/device")
      promise.reject("TORCH_UNAVAILABLE", "Torch is not available on this camera/device")
      return
    }

    try {
      val torchFuture: ListenableFuture<Void> = activeCamera.cameraControl.enableTorch(enabled)
      torchFuture.addListener({
        try {
          ...
          if (isOperationCanceled(error)) {
            // CameraX cancels older torch ops when a newer one is set or camera is reconfiguring.
            promise.resolve(null)
            return@addListener
          }
          val fallbackOk = setTorchWithCameraManagerFallback(enabled)
          if (fallbackOk) {
            ...
            emitError(error.message ?: "Torch operation failed")
            promise.reject("TORCH_FAILED", error.message, error)
          }
        }
      }, ContextCompat.getMainExecutor(context))
    } catch (error: Exception) {
  ...
  private fun bindCamera(provider: ProcessCameraProvider, activity: Activity) {
    ...
    fallbackTorchCameraId = resolveBackCameraWithFlash(context)
  }

  private fun resolveBackCameraWithFlash(appContext: ReactApplicationContext): String? {
    return try {
  ...
  private fun setTorchWithCameraManagerFallback(enabled: Boolean): Boolean {
    return try {
      val camId = fallbackTorchCameraId ?: resolveBackCameraWithFlash(context) ?: return false
      val manager = context.getSystemService(Context.CAMERA_SERVICE) as CameraManager
      manager.setTorchMode(camId, enabled)
      true
    } catch (error: Exception) {
      if (isOperationCanceled(error)) return true
      emitError(error.message ?: "Torch fallback failed")
      false
    }
  }

  private fun isOperationCanceled(error: Exception): Boolean {
    val name = error.javaClass.name
    val msg = (error.message ?: "").lowercase()
    return name.contains("OperationCanceledException") ||
      msg.contains("operationcanceledexception") ||
      msg.contains("camera is not active") ||
      msg.contains("new enable torch being set")
  }
