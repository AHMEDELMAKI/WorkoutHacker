// src/features/workout/screens/ChestWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'ChestWorkout'>;
...
const ChestWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'ChestWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Chest';
