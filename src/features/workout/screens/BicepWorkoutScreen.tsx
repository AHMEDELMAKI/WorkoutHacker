// src/features/workout/screens/BicepWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'BicepWorkout'>;
...
const BicepWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'BicepWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Bicep';
...
export default BicepWorkoutScreen;
