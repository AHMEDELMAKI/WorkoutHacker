// src/features/workout/screens/TrapeziusWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'TrapeziusWorkout'>;
...
const TrapeziusWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'TrapeziusWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Trapezius';
...
export default TrapeziusWorkoutScreen;
