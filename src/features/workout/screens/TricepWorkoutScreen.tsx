// src/features/workout/screens/TricepWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'TricepWorkout'>;
...
const TricepWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'TricepWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Tricep';
...
export default TricepWorkoutScreen;
