// src/features/workout/screens/DeltoidWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'DeltoidWorkout'>;
...
const DeltoidWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'DeltoidWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Deltoid';
...
export default DeltoidWorkoutScreen;
