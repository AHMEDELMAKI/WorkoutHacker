// src/features/workout/screens/LatsWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'LatsWorkout'>;
...
const LatsWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'LatsWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Lats';
...
export default LatsWorkoutScreen;
