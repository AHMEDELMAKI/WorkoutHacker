// src/features/workout/screens/LumbarWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'LumbarWorkout'>;
...
const LumbarWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'LumbarWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Lumbar';
...
export default LumbarWorkoutScreen;
