// src/features/workout/screens/QuadWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'QuadWorkout'>;
...
const QuadWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'QuadWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Quad';
...
export default QuadWorkoutScreen;
