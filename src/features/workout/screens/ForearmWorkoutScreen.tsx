// src/features/workout/screens/ForearmWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'ForearmWorkout'>;
...
const ForearmWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'ForearmWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Forearm';
...
export default ForearmWorkoutScreen;
