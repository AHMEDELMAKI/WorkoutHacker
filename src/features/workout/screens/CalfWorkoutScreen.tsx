// src/features/workout/screens/CalfWorkoutScreen.tsx
...
type Props = NativeStackScreenProps<WorkoutStackParamList, 'CalfWorkout'>;
...
const CalfWorkoutScreen: React.FC<Props> = ({ navigation }) => {
    const route = 'CalfWorkout' as const;
    const category = getCategoryByRoute(route);
    const exercises = getExercisesByRoute(route);

    const title = category?.title ?? 'Calf';
...
export default CalfWorkoutScreen;
